 ...
 * made). Built on `Dialog` (`@moon-ui/modal`'s own shared dialog shell — badge header, proper
 * body/footer padding, desktop `Modal`/mobile `BottomModal` split already handled by it) rather
 * than a bare `Modal` with hand-rolled content: that first pass looked visibly off — `Input`
 * with no `border` prop is the same color as the modal background in light mode
 ...
 * `Dialog`'s own footer already does (see ChecklistFieldGroupAddGroup's own `.secondaryButton`,
 * mirrored below) — this just wasn't doing either yet. */
const NewFolderModal = ({ visible, onDismiss, onCreate }: Props) => {
  ...
  // Dialog only mounts its children while `visible` (Modal/BottomModal both `if (!visible)
  // return null`), so the input doesn't exist yet on the render that flips `visible` true —
  // a rAF gives the real DOM node a beat to land before focusing it.
  React.useEffect(() => {
    if (!visible) return;
