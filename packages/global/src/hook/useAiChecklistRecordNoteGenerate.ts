...
// threaded into NoteEditor's `ai` prop, see that file's own comment), but for a note-type
// field's value on an already-submitted checklist record specifically: `recordId` is captured
// here and forwarded to ai-checklist-record-note, which resolves that value's real existing
// content itself, server-side (see that edge function's own comment for why this is a position,
// not client-sent text). Call site: ChecklistFieldGeneral.tsx.

import { useIsPro } from '../store/pro/useProStatus';
import { generateChecklistRecordNote } from '../store/note/aiChecklistRecordNoteApi';
...
export const useAiChecklistRecordNoteGenerate = (recordId: string) => {
  ...
  const generate = async (
    ...
    const { blocks } = await generateChecklistRecordNote({
      prompt,
      options,
      recordId,
