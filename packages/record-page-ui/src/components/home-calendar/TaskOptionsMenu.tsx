...
// A single "⋮" menu next to View details — holds both of this modal's own per-template actions
// (change calendar color, remove) behind one icon instead of two always-visible controls
// cluttering the header. Change-color is a manual pick from the same fixed 10-swatch palette every
// unset template already falls back into automatically (DEFAULT_PALETTE/hashColor,
// useCalendarEvents.ts), so a chosen color never looks out of place next to an automatic one;
// Remove opens the same This/This-and-following/All scope confirm (DeleteTaskModal)
// checklist-day's own delete flow uses.
//
