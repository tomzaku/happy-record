...
// so a challenge participant PATCHing `/checklist-templates/:id` (the owner's template) with
// `{ repeat: {...} }` sets their own personal reminder time without touching the owner's schedule
// or needing to own the template at all; sending anything else in the same call (title, tags,
// ...) still silently no-ops for them, since that part of the update only ever matches the
// owner's own row.
...
export async function updateChecklistTemplateHandler({ req, db, userId, id }: Ctx) {
  ...
  if ('repeat' in params) {
    await saveRepeat(db, params.repeat, { userId, checklistTemplateId: id });
  }
  return { ok: true };
}
