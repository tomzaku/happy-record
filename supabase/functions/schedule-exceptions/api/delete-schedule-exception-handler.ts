...
// `<checklistTemplateId>:<occurrenceStartedAt>` shape, never the underlying row id directly — see
// index.ts's own comment on why.
...
export async function deleteScheduleExceptionHandler(ctx: Ctx) {
  const { checklistTemplateId, occurrenceStartedAt } = parseCompositeId(ctx.id!);
  await deleteScheduleException(ctx, { checklistTemplateId, occurrenceStartedAt });
  return { ok: true };
}
