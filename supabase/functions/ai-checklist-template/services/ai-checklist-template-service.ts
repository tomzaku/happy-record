// Business logic for `ai-checklist-template`: the caller's own reusable field catalog (fetched
// here, not trusted from the client — see this resource's own index.ts doc comment) and the
// prompt this generation actually sends. No real cross-user visibility decision here (the
// repository already scopes to own rows + anyone's `visibility: 'public'`), so this stays the
// composition layer between `api/` and `repository/ai-checklist-template-repository.ts`.

import { type BuiltRequest, reqStr } from '../../../shared/ai.ts';
import {
  type AvailableField,
  FIELD_TYPES,
  isFieldType,
...
import { fetchFieldsForCatalog } from '../repository/ai-checklist-template-repository.ts';
import type { SupabaseClient } from 'npm:@supabase/supabase-js@2';

/** Same visibility rule `fields`' own unscoped GET uses (own rows + anyone's `visibility:
 * 'public'`) — every real field type, not just number/note (this used to filter down to those
 * two, which meant an existing text/date/datetime field could never be offered for reuse at all)
 * — capped at 60 for prompt-size budgeting, same limit the old client-supplied param used to
 * enforce. */
export async function fetchAvailableFields(db: SupabaseClient, userId: string): Promise<AvailableField[]> {
  const rows = await fetchFieldsForCatalog(db, userId, FIELD_TYPES);
  return rows
    .map((f) => {
...
export function buildGenerationRequest(p: Record<string, unknown>, availableFields: AvailableField[]): BuiltRequest {
